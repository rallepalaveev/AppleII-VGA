This is a redesign of the Apple II VGA card by Mark Aikens.
This version is based on the PR2354B chip with 48 GPIOs (on Clintech Pico Board) which is 5V tolerant and hence no trinsceivers would be needed.

The A2VGA2 hardware design is released under the MIT License:

    Copyright (c) 2026 Ralle Palaveev

The firmware project would be based on awesome work from the following Apple II Analog VGA projects:

    AppleII-VGA. MIT License. Copyright (c) 2021-2022 Mark Aikens.

    Analog Firmware. MIT License. Copyright (c) 2022-2023 David Kuder.
