#pragma once

#define CONFIG_SYSCLOCK 126.0 /* MHz */

// Pin configuration
#ifdef CPB
#define CONFIG_PIN_APPLEBUS_DATA_BASE 0 /* 24+2 pins */
#define CONFIG_PIN_APPLEBUS_RW (CONFIG_PIN_APPLEBUS_DATA_BASE + 24)
#define CONFIG_PIN_APPLEBUS_DEVSEL (CONFIG_PIN_APPLEBUS_DATA_BASE + 25)
#define CONFIG_PIN_APPLEBUS_ADDR(value) (value)
#define CONFIG_PIN_APPLEBUS_DATA(value) (value >> 16)
#define CONFIG_PIN_APPLEBUS_PHI0 26
#define CONFIG_PIN_VSYNC 27
#define CONFIG_PIN_HSYNC 28
#define CONFIG_PIN_RGB_BASE 29 /* 9 pins */
#define CONFIG_PIN_APPLEBUS_SYNC 38
#else
#define CONFIG_PIN_APPLEBUS_DATA_BASE 0 /* 8+2 pins */
#define CONFIG_PIN_APPLEBUS_DEVSEL (CONFIG_PIN_APPLEBUS_DATA_BASE + 8)
#define CONFIG_PIN_APPLEBUS_RW (CONFIG_PIN_APPLEBUS_DATA_BASE + 9)
#define CONFIG_PIN_APPLEBUS_ADDR(value) (value >> 10)
#define CONFIG_PIN_APPLEBUS_DATA(value) (value)
#define CONFIG_PIN_APPLEBUS_SYNC 10
#define CONFIG_PIN_APPLEBUS_CONTROL_BASE 11 /* 3 pins */
#define CONFIG_PIN_APPLEBUS_PHI0 26
#define CONFIG_PIN_HSYNC 28
#define CONFIG_PIN_VSYNC 27
#define CONFIG_PIN_RGB_BASE 14 /* 9 pins */
#endif

// Other resources
#define CONFIG_VGA_PIO pio0
#define CONFIG_VGA_SPINLOCK_ID 31
#define CONFIG_ABUS_PIO pio1
